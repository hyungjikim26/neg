...
LR         = 2e-5
MAX_LEN    = 512
STS_CHECKPT= "./stsb_roberta_regressor/checkpoint-1800"
OUT_DIR    = "./roberta_nevir_pairwise"
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in tqdm(train_loader, desc=f"Train {epoch}"):
            ...
            out1 = model(
                ...
                attention_mask=batch["attn_mask_1"],
            ).logits.squeeze(-1)
            out2 = model(
                input_ids=batch["input_ids_2"],
                attention_mask=batch["attn_mask_2"],
            ).logits.squeeze(-1)
            logits = torch.stack([out1, out2], dim=1)  # (B,2)
